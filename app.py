...
def main_short_url(email):
    ...
    try:
        # original_url = f"http://172.23.21.248:8080/spam_waringing/search?mail={email}"  # URL 포맷
        original_url = f"http://127.0.0.1:5000/spam_warning/search?mail={email}"  # URL 포맷
        shortener = pyshorteners.Shortener()
        short_url = shortener.tinyurl.short(original_url)
        return jsonify({
            "original_url": original_url,
